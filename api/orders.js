    ...
    try {
      ...
      if(req.method==='GET') {
        ...
        return res.json({ok:true,data,people:ctx.roster.map(({id,name})=>({id,name})),stages:schema.stages,
          ...
          connection:{read:true,writeEnabled:env.ORDER_WRITES_ENABLED==='true',directoryEnabled:env.ORDER_DIRECTORY_ENABLED==='true',missing,stageMirror:S.stageMirrorReady(ctx),checkedAt:Date.now()},permissions:{write:user.quyen.ghi_order===true,review:user.quyen.duyet_order===true}});
      }
