...
async function update(client,ctx,user,body) {
  ...
  if(team!=='shoots' && task.historyInvalid)throw fail(409,'Cột Lịch sử không đúng định dạng. Đã dừng để tránh ghi đè lịch sử hiện có.');
  ...
  else values.history=journal(task,user,action,detail);
