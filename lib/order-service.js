...
// Cột soi gương là tuỳ chọn: thiếu nó thì vẫn ghi được, nhưng phải nói ra
// chứ không im lặng — im lặng chính là cái bẫy đã làm mất công tìm một lần.
function stageMirrorReady(ctx) {
  const [name,type]=S.schemas.media.stageText;
  const f=(ctx.tables.media?.fields||[]).find(f=>f.field_name===name);
  return !f?'missing':f.type!==type?'type':'ok';
}
...
async function create(client,ctx,user,body) {
  ...
  if(team==='shoots') {
    ...
    if(team==='media') {
      ...
      syncStage(team,values,values.stage,metadata);
...
module.exports={context,requirements,stageMirrorReady,expose,create,update,creationKey,approved,editable,assertStage};
