...
const PHAN_LOAI = [
  ...
  { ten: 'ĐƠN CHUYỂN PHÁT',    xepNguoi: false, canGio: false, coCod: true,  ti: 0.10 },
  { ten: 'KHÁCH LẤY TẠI KHO',  xepNguoi: false, canGio: false, coCod: true,  ti: 0.05 },
  { ten: 'HÀNG TRƯNG BÀY',     xepNguoi: false, canGio: false, coCod: false, ti: 0.03 },
];
...
const HINH_THUC = ['Tiền mặt', 'Chuyển khoản về công ty', 'Quét QR'];
...
function duLieuMinhHoa() {
    ...
    for (let i = 0; i < soDon; i++) {
      ...
      if (!pl.xepNguoi) {
        ...
        if (rnd() < 0.32) {
          ...
          hinhThuc = chon(HINH_THUC);
          if (lech === -2) phien = 'PH-2609-01';
        }
      } else if (lech === 0) {
        const b = rnd();
        trangThai = b < 0.18 ? 'Đang giao' : b < 0.4 ? 'Chờ xuất kho' : b < 0.82 ? 'Đã xếp người' : 'Chờ xác nhận';
      ...
      don.push({
        ...
        quan: rnd() < 0.78 ? chon(quanNong[khuVuc]) : chon(QUAN[khuVuc]), diaChi: 'Căn hộ ' + (100 + Math.floor(rnd() * 800)) + ', ' + chon(DUONG),
  ...
  return { don, nguoiGiao: NGUOI_GIAO, khungGio: KHUNG, phanLoai: PHAN_LOAI };
}
