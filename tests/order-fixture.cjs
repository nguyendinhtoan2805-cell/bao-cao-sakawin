...
const {schemas,missing}=require('../lib/order-schema.js');
...
function fixture(){
  ...
  for(const team of Object.keys(schemas))tables[team]={fields:Object.entries(schemas[team]).map(([key,[field_name,type]],i)=>({field_name,type,field_id:'fldDemo'+i,property:{...(type===18?{table_id:'tblShoots'}:{}),options:(key==='legacyStage'?(team==='media'?['Chưa thực hiện','Đang dựng','Hoàn thành']:['Chưa làm','Đang làm','Hoàn thành']):key==='requester'?['Content A','Content B']:key==='channels'?['Facebook','TikTok','YouTube']:key==='priority'?['Bình thường','Gấp']:key==='month'?['T8.2026','T9.2026','T10.2026']:[]).map(name=>({name}))}})),records:[]};
  ...
  for(let i=0;i<8;i++)tables.media.records.push(raw('media','recMedia'+i,{title:titles[i],code:'M-00'+(i+1),month:'T9.2026',brief:'Nội dung minh hoạ để kiểm tra bố cục và quy trình.',scriptLink:{text:'Kịch bản minh hoạ',link:'https://example.com/script/'+i},requester:[people[i%2]],writers:[people[i%2]],assignees:[people[2+i%2]],channels:['Facebook','TikTok'],createdAt:stamp('2026-09-01T09:00:00'),deadline:stamp('2026-09-'+String(10+i).padStart(2,'0')+'T23:59:00'),stage:stages[i],legacyStage:i===7?'Hoàn thành':i>=4?'Đang dựng':'Chưa thực hiện',shoot:i===0?[]:['recShoot'+(i===1?2:i<4?1:0)],importantScript:i===1,importantFinal:i===6,hours:i%2?null:4,result:i>=4?{text:'Video minh hoạ',link:'https://example.com/video/'+i}:null,completedAt:i===7?stamp('2026-09-08T15:00:00'):null,history:'[]'}));
  for(let i=0;i<6;i++)tables.design.records.push(raw('design','recDesign'+i,{title:['Bộ ảnh giới thiệu sản phẩm','Banner chiến dịch tháng 9','Ảnh hướng dẫn lắp đặt','Ảnh bìa Fanpage','Bộ ảnh thông số','Hình sản phẩm mới'][i],code:'D-00'+(i+1),createdAt:stamp('2026-09-01T09:00:00'),deadline:stamp('2026-09-15T23:59:00'),requester:'Content A',assignees:[people[2+i%2]],stage:['Chờ kiểm tra brief','Đã nhận order','Đang thiết kế','Cần sửa','Chờ duyệt thành phẩm','Hoàn thành'][i],legacyStage:i===5?'Hoàn thành':'Chưa làm',style:'Gọn, rõ thông tin, theo bộ nhận diện Sakawin.',size:'1080 × 1080',quantity:3,hours:3,result:i>=4?{link:'https://example.com/design/'+i,text:'Ảnh minh hoạ'}:null,importantFinal:i===4,completedAt:i===5?stamp('2026-09-08T15:00:00'):null,history:'[]'}));
  const calls=[];
