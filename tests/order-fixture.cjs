...
function fixture(){
  ...
  const raw=(team,id,values)=>{
    const row={record_id:id,fields:Object.fromEntries(Object.entries(values).filter(([k])=>schemas[team][k]).map(([k,v])=>[schemas[team][k][0],v]))};
    if(team==='media') {
      const task={...decode(team,row),...Object.fromEntries(Object.entries(values).filter(([k])=>!schemas.media[k]))};task.history=[];
      row.fields['Lịch sử']=JSON.stringify([{at:stamp('2026-09-01T09:00:00'),by:'demo.a@example.test',action:'create',detail:{mediaState:require('../lib/order-schema.js').mediaState(task)}}]);
    }
    return row;
  };
