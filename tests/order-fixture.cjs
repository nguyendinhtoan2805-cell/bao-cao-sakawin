...
const {schemas,missing,scriptApprovalOptions}=require('../lib/order-schema.js');
...
function fixture(){
  ...
  for(const team of Object.keys(schemas))tables[team]={fields:Object.entries(schemas[team]).map(([key,[field_name,type]],i)=>({field_name,type,field_id:'fldDemo'+i,property:{...(type===18?{table_id:'tblShoots'}:{}),options:(key==='scriptApproval'?scriptApprovalOptions:key==='legacyStage'?(team==='media'?['Chưa thực hiện','Đang dựng','Hoàn thành']:['Chưa làm','Đang làm','Hoàn thành']):key==='requester'?['Content A','Content B']:key==='channels'?['Facebook','TikTok','YouTube']:key==='priority'?['Bình thường','Gấp']:key==='month'?['T8.2026','T9.2026','T10.2026']:[]).map(name=>({name}))}})),records:[]};
